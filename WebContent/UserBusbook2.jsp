...
<%
	...
	String query = "Select * FROM Bus WHERE FRO=? and T=? and DOJ=?";
	PreparedStatement prepStmt = null;
	ResultSet rs = null;
	prepStmt = conn.prepareStatement(query);
	prepStmt.setString(1, fro);
	prepStmt.setString(2, to);
	prepStmt.setString(3, doj);
...
<table>
  <tr>
	<td><%=i++%></td>  
    <td style="color:red;text-align:center;"><%=rs.getString("BusName")%></td>
    <td style="color:blue;"><%=rs.getString("Departure") %></td>
    <td style="color:green;"><%=rs.getString("Arrival") %></td>
        <td><%=rs.getString("Price") %></td>
        <td><a href="seatview.jsp"><img src="http://www.skyvalley4u.com/Logos/booknowAnimated.gif" width="100px" height="50px" style="float:right;"></a></td>
  </tr>
  </table>
  <%}while(rs.next());}
